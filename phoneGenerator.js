 ...
 * Generates valid US phone numbers with real area codes
 */
...
const areaCodes = [
    ...
    '657', '660', '661', '662', '667', '669', '678', '680', '681', '682',
    ...
    '917', '918', '919', '920', '925', '928', '929', '931', '936', '937',
 ...
 * @param {string} format - Format: 'dashes', 'dots', 'plain', 'parentheses'
 * @returns {string}
 */
export function generatePhone(format = 'plain') {
...
/**
 * Generate phone with specific area code
 * @param {string} areaCode 
 * @param {string} format 
 * @returns {string}
 */
export function generatePhoneWithAreaCode(areaCode, format = 'plain') {
    const exchange = String(Math.floor(Math.random() * 900) + 100);
    const subscriber = String(Math.floor(Math.random() * 9000) + 1000);
...
export default { generatePhone, generatePhoneWithAreaCode, getAreaCodes, isValidPhone };
